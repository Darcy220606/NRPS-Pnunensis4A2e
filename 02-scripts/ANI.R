...
######################################################
# Set-WD ####
setwd("F:/pseudomonas_Spflanze/04-analysis/tree_complete_genome/ANI")
######################################################
...
pims <- read.table("ANI_OUT_MATRIX_fg3000.txt", header=TRUE, sep="\t",row.names = 1)

indx <- sapply(pims, is.factor)
pims[indx] <- lapply(pims[indx], function(x) as.numeric(as.character(x)))
